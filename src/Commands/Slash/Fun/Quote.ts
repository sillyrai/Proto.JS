...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("quote")
        .setDescription("📜 Generate an incorrect quote from one or more people (up to 6 people!)")
        .addStringOption(option =>
            option.setName("name1")
                .setDescription("The name of the first person to quote")
                .setRequired(true))
        .addStringOption(option =>
            option.setName("name2")
                .setDescription("The name of the second person to quote")
                .setRequired(false))
        .addStringOption(option =>
            option.setName("name3")
                .setDescription("The name of the third person to quote")
                .setRequired(false))
        .addStringOption(option =>
            option.setName("name4")
                .setDescription("The name of the fourth person to quote")
                .setRequired(false))
        .addStringOption(option =>
            option.setName("name5")
                .setDescription("The name of the fifth person to quote")
                .setRequired(false))
        .addStringOption(option =>
            option.setName("name6")
                .setDescription("The name of the sixth person to quote")
                .setRequired(false))
        .setContexts([
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        // Depending on how many names have been passed in the command arguments, we choose from the appropriate list of prompts
        let name1 = interaction.options.getString("name1", true)
        let name2 = interaction.options.getString("name2", false) || null
        let name3 = interaction.options.getString("name3", false) || null
        let name4 = interaction.options.getString("name4", false) || null
        let name5 = interaction.options.getString("name5", false) || null
        let name6 = interaction.options.getString("name6", false) || null

        let prompt = ""

        if (name6) {
            prompt = prompts6[Math.floor(Math.random() * prompts6.length)]
            prompt = prompt.replace(/{A}/g, `**${name1}**`).replace(/{B}/g, `**${name2!}**`).replace(/{C}/g, `**${name3!}**`).replace(/{D}/g, `**${name4!}**`).replace(/{E}/g, `**${name5!}**`).replace(/{F}/g, `**${name6!}**`)
        } else if (name5) {
            prompt = prompts5[Math.floor(Math.random() * prompts5.length)]
            prompt = prompt.replace(/{A}/g, `**${name1}**`).replace(/{B}/g, `**${name2!}**`).replace(/{C}/g, `**${name3!}**`).replace(/{D}/g, `**${name4!}**`).replace(/{E}/g, `**${name5!}**`)
        } else if (name4) {
            prompt = prompts4[Math.floor(Math.random() * prompts4.length)]
            prompt = prompt.replace(/{A}/g, `**${name1}**`).replace(/{B}/g, `**${name2!}**`).replace(/{C}/g, `**${name3!}**`).replace(/{D}/g, `**${name4!}**`)
        } else if (name3) {
            prompt = prompts3[Math.floor(Math.random() * prompts3.length)]
            prompt = prompt.replace(/{A}/g, `**${name1}**`).replace(/{B}/g, `**${name2!}**`).replace(/{C}/g, `**${name3!}**`)
        } else if (name2) {
            prompt = prompts2[Math.floor(Math.random() * prompts2.length)]
            prompt = prompt.replace(/{A}/g, `**${name1}**`).replace(/{B}/g, `**${name2!}**`)
        }
        else {
            prompt = prompts1[Math.floor(Math.random() * prompts1.length)]
            prompt = prompt.replace(/{A}/g, `**${name1}**`)
        }

        let Response = new ContainerBuilder();
        Response.addTextDisplayComponents(new TextDisplayBuilder().setContent(`## :scroll: Incorrect quote!`))
        Response.addSeparatorComponents(new SeparatorBuilder())
        Response.addTextDisplayComponents(new TextDisplayBuilder().setContent(prompt))
